...
public class BFDSearchImpl implements BFDSearch {
    ...
    public IBaseBundle searchEOB(long patientId, OffsetDateTime since, OffsetDateTime until, int pageSize, String bulkJobId, FhirVersion version, String contractNum) throws IOException {
        ...
        HttpPost request = getHttpPostRequest(patientId, since, until, pageSize, bulkJobId, version, contractNum);
    ...
    @Trace
    @Override
    public IBaseBundle searchEOBString(String patientId, String since, String until, int pageSize, String bulkJobId, FhirVersion version, String contractNum, String startIndex) throws IOException {

        HttpPost request = getHttpPostRequestString(patientId, since, until, pageSize, bulkJobId, version, contractNum, startIndex);

        log.info("Executing BFD Search Request {}", request);

        byte[] responseBytes = getEOBSFromBFD(Long.parseLong(patientId), request);

        return parseBundle(version, responseBytes);
    }


    private HttpPost getHttpPostRequest(long patientId, OffsetDateTime since, OffsetDateTime until, int pageSize, String bulkJobId, FhirVersion version, String contractNum) throws UnsupportedEncodingException {
        ...
        log.info("Executing searchEOB request " + request);
        return request;
    }

    private HttpPost getHttpPostRequestString(String patientId, String since, String until, int pageSize, String bulkJobId, FhirVersion version, String contractNum, String startIndex) throws UnsupportedEncodingException {
        StringBuilder url = new StringBuilder(bfdClientVersions.getUrl(version) + "ExplanationOfBenefit/_search");
